...
const AwsMigratePage = () => {
  //find a way to authenticate 
  ...
  const handleMigration = async () => {
    ...
    // todo: handle accessing secret keys from .env
    // todo: validate incoming form data
    ...
    try {
      //handle localhost1337 gracefully
  ...
  return (
    <Box padding={10} shadow="filterShadow">
      ...
      <div>
        <Box padding={4}>
        {/* //todo  aws documentation hyperlink for creating and using keys*/}
      ...
      <Box padding={4}>
        ...
        {/* <Button variant="default" onClick={handleMigration} loading={migrationStarted}>
          {migrationStarted ? `Migrating` : `Start Migration`}
        </Button> */}
      </Box>
    </Box>
  );
