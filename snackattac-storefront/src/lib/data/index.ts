...
/**
 * @returns blog posts from strapi
 */
export async function getAllBlogs() {
  const { data } = await fetch(`${CMS_BASE_URL}/api/articles`)
    .then((res) => res.json())
    .catch((err) => {
      throw err
    })
...
/**
 * @returns blog posts based on slug from strapi
 */
export async function getBlogsBySlug(slug) {
  console.log("called : : ")
  const data = await fetch(`${CMS_BASE_URL}/api/article/${slug}`)
    .then((res) => res.json())
    .catch((err) => {
      throw err
    })
